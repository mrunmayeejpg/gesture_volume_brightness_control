...
my_hands = mpo.solutions.hands.Hands()
drawing_utils = mpo.solutions.drawing_utils

while True:
    _, image = webcam.read()
    image = cv2.flip(image, 1)
    frame_height, frame_width, _ = image.shape
    rgb_image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)#converts to
        ...
        for hand in hands:
            ...
            for id, landmark in enumerate(landmarks):
                ...
                if id == 20:  # Pinky fingertip
                    cv2.circle(img=image, center=(x, y), radius=8, color=(0, 255, 255), thickness=3)
                    x5, y5 = x, y
        #1-Index 2-Thumb 3-Middle 4-Ring 5-Pinky
        # Calculate distances
